...
### 13. CLAUDE.md 수정 정책
- `CLAUDE.md` 파일은 **직접 수정 금지**
- 개발 규칙/철학 변경은 `AGENTS.md`에만 반영
...
### 프로젝트 개요 (요약)
- 목적: 도시계획 주민참여 토의를 LLM 다중 에이전트로 시뮬레이션하고, 기술 구현 차이에 따른 포용성 변화를 비교
- 실험 레벨: Lv.1(단일 세션/단일 모델), Lv.2(개별 컨텍스트/단일 모델), Lv.3(개별 컨텍스트/복수 모델), Lv.4(Lv.3 + 사회자)
- 기본 흐름: 페르소나 생성 → 서사 생성 → 초기 입장 형성 → 라운드 토의 → 최종 발언 → 도시계획가 합의문 → 주민 만족도 투표
- 주요 산출물: `outputs/`에 토의/생각/토큰 CSV, 에이전트 목록 CSV, 합의문 JSON 저장
- 핵심 입력: `prompts/` 규칙/맥락 파일, `prompts/vulnerable/` 취약 페르소나, `debate/config.py` 분포 설정
